...
def flush_batch_aggregators(batch_idx, batch_agg, batch_contingency):
    """
    Writes the batch aggregator data to CSV files and returns empty dictionaries.
    
    Args:
        batch_idx (int): The current batch index.
        batch_agg (dict): Mapping original bus id to list of (OTDF contribution, circuit_debug).
        batch_contingency (dict): Mapping branch name to list of contingency tuples.
        
    Returns:
        Tuple of empty dictionaries for batch_agg and batch_contingency.
    """
    # Flush OTDF aggregator for the batch.
    if batch_agg:
        df_batch = pd.DataFrame([
            {"Bus_ID": bus, "scaled_OTDF_Contribution": scaled_contrib,"Circuit_Debug": debug}
            for bus, entries in batch_agg.items() for scaled_contrib, debug in entries
        ])
        agg_file = os.path.join(RESULTS_DIR, f"agg_batch_{batch_idx:03d}.csv")
        df_batch.to_csv(agg_file, index=False)
    ...
    if batch_contingency:
        df_cont = pd.DataFrame([
            ...
            for branch, entries in batch_contingency.items() for intact, post, circuit in entries
        ])
        cont_file = os.path.join(RESULTS_DIR, f"contingency_batch_{batch_idx:03d}.csv")
    ...
    # Return empty dictionaries to clear the batch data.
    return {}, {}
    ...
    for f in batch_files:
        ...
        for j in threshold_idx:
            ...
            for b_idx in nz:
                ...
                contrib_records.append((orig_bus_id, sc, circuit_debug))
        
        # Convert the records to a DataFrame for efficient grouping.
        if contrib_records:
            df_contrib = pd.DataFrame(contrib_records, columns=["Bus_ID", "Scaled Contribution","Circuit_Debug"])
            ...
            for bus, group in df_contrib.groupby("Bus_ID"):
                ...
                batch_agg.setdefault(bus, []).extend(list(group[["Scaled Contribution", "Circuit_Debug"]].itertuples(index=False, name=None)))
...
def merge_global_aggregators():
    ...
    if not agg_files:
        ...
        circ = df.loc[idxmax, ["Bus_ID","Bus_Name","Circuit_Debug","scaled_OTDF_Contribution"]]
        circ = circ.rename(columns={"Circuit_Debug":"Circuit_Involved_max",
                                    "scaled_OTDF_Contribution": "Scaled OTDF_Contribution_max"})
        circm = df.loc[idxmin, ["Bus_ID","Bus_Name","Circuit_Debug","scaled_OTDF_Contribution"]]
        ...
        df_agg_final1.to_csv(out1, index=False)
